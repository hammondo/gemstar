...
const SERVICE_NAME_MAP: Record<string, string> = {
  ...
  "shrinking violet infrared sauna pod treatment":
    "infrared_sauna_shrinking_violet",
...
export class FreshaWatcherAgent {
  ...
  parseCsv(csvPath: string): Record<string, number> {
    ...
    if (lines.length < 2) {
      console.warn(
        "[FreshaWatcher] CSV appears empty or has only a header row",
      );
      return {};
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const statusRaw =
        statusCol !== -1
          ? (cols[statusCol] ?? "").trim().toLowerCase()
          : "booked";

      // Skip non-booked statuses (cancelled, no-show, completed, etc.)
      if (statusRaw && !BOOKED_STATUSES.some((s) => statusRaw.includes(s)))
        continue;
  ...
  private computeSignals(
    ...
    for (const service of this.services) {
      ...
      const capacity =
        (service as unknown as Record<string, number>)["weeklyCapacity"] ??
  ...
  private parseDate(dateStr: string): Date | null {
    ...
    if (dmy2)
      return new Date(
        `${dmy2[3]}-${dmy2[2].padStart(2, "0")}-${dmy2[1].padStart(2, "0")}`,
      );
