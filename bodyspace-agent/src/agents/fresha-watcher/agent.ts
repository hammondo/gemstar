...
export class FreshaWatcherAgent {
  ...
  async run(): Promise<AvailabilitySignals> {
    ...
    const push  = Object.values(signals).filter((v) => v.signal === "push").map((v) => v.serviceName);
    const pause = Object.values(signals).filter((v) => v.signal === "pause").map((v) => v.serviceName);
  ...
  parseCsv(csvPath: string): Record<string, number> {
    ...
    console.log(`[FreshaWatcher] Found ${totalBookings} confirmed bookings across ${Object.keys(counts).length} services`);
    return counts;
  }
  ...
  private computeSignals(bookingCounts: Record<string, number>): AvailabilitySignals {
    ...
    for (const service of this.services) {
      ...
      const capacity = (service as unknown as Record<string, number>)["weeklyCapacity"]
        ?? (service.pushThreshold + 4);
  ...
  private async tryAutoDownload(): Promise<void> {
    if (!process.env.FRESHA_EMAIL || !process.env.FRESHA_PASSWORD) return;
    ...
    try {
      ...
      console.warn(`[FreshaWatcher] Auto-download unavailable (${String(err).slice(0, 80)}) — using existing CSV or mock data`);
    }
  }
}

// Run standalone:
//   tsx src/agents/fresha-watcher/agent.ts                   (auto-find latest CSV)
//   tsx src/agents/fresha-watcher/agent.ts path/to/file.csv  (use specific file)
if (process.argv[1].endsWith("agent.ts") || process.argv[1].endsWith("agent.js")) {
  ...
  if (process.argv[2]?.endsWith(".csv")) {
    const counts = agent.parseCsv(process.argv[2]);
    console.log("\nBooking counts:", counts);
  } else {
    agent.run().then((signals) => console.log(JSON.stringify(signals, null, 2)));
  }
}
