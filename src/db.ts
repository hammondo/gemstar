...
import type {
  Campaign, SocialPost, ServiceAvailabilityData,
  TrendsBrief, CampaignStatus, PostStatus,
} from "./types.js";
...
export function saveAvailabilitySignals(signals: Record<string, ServiceAvailabilityData>): void {
  ...
  const insertMany = db.transaction((rows: ServiceAvailabilityData[]) => {
    for (const row of rows) {
      insert.run(
        randomUUID(), row.serviceId, row.serviceName,
        row.availableSlots, row.totalSlots ?? null, row.signal,
        row.pushThreshold, row.pauseThreshold, j(row),
      );
    }
  });
...
export function saveTrendsBrief(data: Omit<TrendsBrief, "id" | "createdAt">): TrendsBrief {
  ...
  db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?)
  `).run(
    id, data.weekOf, data.competitorSummary, data.trendSignals,
    data.seasonalFactors, data.recommendedFocus, data.opportunities,
    j(data.sources), data.confidence, j(data),
  );
...
export function getLatestTrendsBrief(): TrendsBrief | null {
  ...
  const row = db.prepare(
    "SELECT * FROM trends_briefs ORDER BY created_at DESC LIMIT 1"
...
export function saveCampaign(data: Omit<Campaign, "id" | "createdAt">): Campaign {
  ...
  db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?)
  `).run(
    id, data.name, data.theme, data.description,
    j(data.targetServices), data.durationWeeks,
    data.status, j(data.freshaSignals),
    data.trendsBriefId ?? null, now, now,
  );
  ...
  const insertPosts = db.transaction((posts: SocialPost[]) => {
    for (const post of posts) {
      insertPost.run(
        post.id, id, post.platform, post.postType,
        post.contentPillar, post.copy, post.imageDirection,
        j(post.hashtags), post.callToAction,
        post.scheduledFor ?? null, post.status,
      );
    }
  });
...
export function getCampaignById(id: string): Campaign | null {
  ...
  const posts = db.prepare("SELECT * FROM social_posts WHERE campaign_id = ? ORDER BY scheduled_for").all(id) as Array<Record<string, unknown>>;
...
export function getCampaignsByStatus(status: CampaignStatus): Campaign[] {
  ...
  return rows.map((row) => {
    const posts = db.prepare(
      "SELECT * FROM social_posts WHERE campaign_id = ? ORDER BY scheduled_for"
...
export function updatePostStatus(
  ...
  `).run(
    status,
    extra.ownerEdit ?? null,
    extra.rejectionReason ?? null,
    extra.postizPostId ?? null,
    postId,
  );
}
